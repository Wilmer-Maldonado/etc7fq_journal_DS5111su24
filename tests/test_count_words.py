...
def test_count_words_key_type():
    ...
    assert all(isinstance(word, str) for word in output.keys()), "All keys in output dict should be strings"
...
def test_count_words_value_type():
    ...
    assert all(isinstance(count, int) for count in output.values()), "All values in output dict should be integers"
...
def test_count_words_full_match():
    ...
    expected_output = {'but': 1, 'the': 2, 'raven': 1, 'sitting': 1, 'lonely': 1, 'on': 1, 'placid': 1, 'bust': 1, 'spoke': 1, 'only': 1, 'that': 2, 'one': 2, 'word': 2, 'as': 1, 'if': 1, 'his': 1, 'soul': 1, 'in': 1, 'he': 1, 'did': 1, 'outpour': 1}
    # Assertion Flags
    assert output == expected_output, "The test output and expected dictionary did not have matching word counts"
...
def test_count_words_full_match_error():
    ...
    expected_output = {'but': 3, 'the': 2, 'raven': 1, 'sitting': 1, 'lonely': 1, 'on': 1, 'placid': 1, 'bust': 1, 'spoke': 1, 'only': 1, 'that': 2, 'one': 2, 'word': 2, 'as': 1, 'if': 1, 'his': 1, 'soul': 1, 'in': 1, 'he': 1, 'did': 1, 'outpour': 1}
    # Assertion Flags
...
def test_count_words_bash_vs_python():
    ...
    bash_output_hello = subprocess.check_output(bash_test_hello, shell=True, text=True).strip()
     # When using sub process to execute bash command on test_string
    bash_test_world = f"echo {test_string} | grep -o 'world' | wc -l"
    bash_output_world = subprocess.check_output(bash_test_world, shell=True, text=True).strip()
     # When using sub process to execute bash command on test_string
    bash_test_python = f"echo {test_string} | grep -o 'python' | wc -l"
    bash_output_python = subprocess.check_output(bash_test_python, shell=True, text=True).strip()
