...
def test_clean_text_type():
    ...
    assert isinstance(output, str), f"Output should be a string, but received {type(output)}"
...
def test_clean_text_full_match():
    ...
    assert output == expected_cleaned_text, f"""The test and expected output did not match exactly, check lowercase/punctuation"""
...
def test_clean_text_bash_vs_python():
    ...
    assert python_output == bash_output, "bash and python outputs on test string do not match"